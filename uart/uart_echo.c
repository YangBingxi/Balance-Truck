...
//    UART1Send(ReciveData_UART1, ReciveData_i_UART1);
    if(ReciveData_UART1[0]=='F')
    {
        ...
        FlagSend = 1;
        MotorOrderDirection = 1;//前：1  后：0  左：2  右： 3
        timeFlag = 1;
    ...
    else if (ReciveData_UART1[0]=='B')
    {
        ...
        TimerEnable(TIMER0_BASE, TIMER_A);
        FlagSend = 1;
        MotorOrderDirection = 0;//前：1  后：0  左：2  右： 3
        timeFlag = 1;
    ...
    {
        Direction_Set = 1;
        Set_Low_Speed = 1;//开启慢速

        Counter = 0; //计数清零
        TimerEnable(TIMER0_BASE, TIMER_A);
        FlagSend =1;
        MotorOrderDirection = 2;//前：1  后：0  左：2  右： 3
        timeFlag = 1;
    ...
    else if (ReciveData_UART1[0]=='R')
    {
        ...
        TimerEnable(TIMER0_BASE, TIMER_A);
        FlagSend = 1;
        MotorOrderDirection = 3;//前：1  后：0  左：2  右： 3
        timeFlag = 1;
    ...
    else if (ReciveData_UART1[0]=='M')
    {

        Car_Mode = (ReciveData_UART1[1]-48);
...
UART2IntHandler(void)
{
    ...
    while(UARTCharsAvail(UART2_BASE))
    {
        ...
        if(ReciveData_UART2[0]==0x5A&&ReciveData_UART2[1]==0x5A)
        {
            Height = (ReciveData_UART2[4]<<8)|ReciveData_UART2[5];
        }
    }
